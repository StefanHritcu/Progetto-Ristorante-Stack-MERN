...
const InFourDay = () => {
  ...
  const [reservationInFourDay, setReservationInFourDay] =
  useState(0);
const [peopleInFourDay, setPeopleInFourDay] = useState(0);
...
useEffect(() => {
  ...
  const totalePersoneInFourDay = peopleInFourDay;
  const totaleCostoPerInFourDay = (costoPerCliente * totalePersoneInFourDay).toFixed(0)
  setCostPerDay(totaleCostoPerInFourDay);
}, [peopleInFourDay]);
...
useEffect(() => {
  ...
  const totalePersoneInFourDay = peopleInFourDay;
  const totaleProfittoPerInFourDay = (totalePersoneInFourDay * profittoPerCliente).toFixed(0)
  setProfitPerDay(totaleProfittoPerInFourDay);
}, [peopleInFourDay]);

  const fetchInFourDay = async () => {
    try {
      const response = await axios.get(
        "http://localhost:5000/admindashboard/inFourDay"
      );
 ...
 const handleEmail = async (prenotazioneEmail) => {
  ...
  //! GET TOTAL RESERVATIONS FROM IN FOUR DAY
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5000/admindashboard/inFourDayReservations"
        );
        console.log("controllo prenotazioni per IN FOUR DAY:", response);
        
        setReservationInFourDay(
          response.data.totalReservationsInFourDays
        );
      } catch (error) {
        console.error(
          "Errore durante il recupero delle prenotazioni per IN FOUR DAY:",
          error
        );
      }
    };
    ...
    //! GET TOTAL PEOPLES FROM IN FOUR DAY
    useEffect(() => {
      const fetchData = async () => {
        try {
          const response = await axios.get(
            "http://localhost:5000/admindashboard/inFourDayPeoples"
          );
          console.log("controllo persone per IN FOUR DAY:", response);
          setPeopleInFourDay(response.data.totalPeoples);
        } catch (error) {
          console.error(
            "Errore durante il recupero delle persone per IN FOUR DAY:",
            error
          );
        }
      };
