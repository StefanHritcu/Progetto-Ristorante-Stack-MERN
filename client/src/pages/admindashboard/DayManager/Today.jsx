...
const Today = () => {
  ...
  const [reservationToday, setReservationToday] = useState(0);
  ...
  useEffect(() => {
    ...
    const totalePersoneToday = peopleToday;
    const totaleCostoPerToday = (costoPerCliente * totalePersoneToday).toFixed(
      0
    );
    setCostPerDay(totaleCostoPerToday);
  }, [peopleToday]);
  ...
  useEffect(() => {
    ...
    const totalePersoneToday = peopleToday;
    const totaleProfittoPerToday = (
      totalePersoneToday * profittoPerCliente
    ).toFixed(0);
    setProfitPerDay(totaleProfittoPerToday);
  }, [peopleToday]);
  ...
  const fetchTodayData = async () => {
    try {
      const response = await axios.get(
        "http://localhost:5000/admindashboard/todayDatas"
      );
      ...
      console.error(
        "Errore durante il recupero delle prenotazioni di oggi:",
        error
      );
    }
  };
  ...
  //! HANDLE PRENOTAZIONE CONFERMATA
  ...
  //! HANDLE PRENOTAZIONE NON CONFERMATA
  ...
  //! GET TOTAL RESERVATIONS FROM TODAY
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5000/admindashboard/todayReservations"
        );
        console.log("controllo prenotazioni per TODAY", response);
        setReservationToday(response.data.totalReservations);
      } catch (error) {
        console.error(
  ...
  //! GET TOTAL PEOPLES FROM TODAY
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5000/admindashboard/todayPeoples"
        );
        console.log("controllo persone per TODAY", response);
        setPeopleToday(response.data.totalPeoples);
      } catch (error) {
        console.error(
          "Errore durante il recupero delle persone per TODAY:",
          error
        );
      }
    };
