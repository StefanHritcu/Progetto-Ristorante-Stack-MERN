...
const InThreeDay = () => {
  ...
  const [reservationInThreeDay, setReservationInThreeDay] =
  useState(0);
const [peopleInThreeDay, setPeopleInThreeDay] = useState(0);
...
useEffect(() => {
  ...
  const totalePersoneInThreeDay = peopleInThreeDay;
  const totaleCostoPerInThreeDay = (costoPerCliente * totalePersoneInThreeDay).toFixed(0)
  setCostPerDay(totaleCostoPerInThreeDay);
}, [peopleInThreeDay]);
...
useEffect(() => {
  ...
  const totalePersoneInThreeDay = peopleInThreeDay;
  const totaleProfittoPerInThreeDay = (totalePersoneInThreeDay * profittoPerCliente).toFixed(0)
  setProfitPerDay(totaleProfittoPerInThreeDay);
}, [peopleInThreeDay]);

  const fetchInThreeDay = async () => {
    try {
      const response = await axios.get(
        "http://localhost:5000/admindashboard/inThreeDay"
      );
      ...
      console.error(
        "Errore durante il recupero delle prenotazioni di dopo domani:",
        error
      );
    }
  };
 ...
 const handleEmail = async (prenotazioneEmail) => {
  ...
  //! GET TOTAL RESERVATIONS FROM IN THREE DAY
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5000/admindashboard/inThreeDayReservations"
        );
        console.log("controllo prenotazioni per IN THREE DAY", response);
        
        setReservationInThreeDay(
          response.data.totalReservationsInThreeDays
        );
      } catch (error) {
        console.error(
          "Errore durante il recupero delle prenotazioni per IN THREE DAY:",
          error
        );
      }
    };
    ...
    //! GET TOTAL PEOPLES FROM IN THREE DAY
    useEffect(() => {
      const fetchData = async () => {
        try {
          const response = await axios.get(
            "http://localhost:5000/admindashboard/inThreeDayPeoples"
          );
          console.log("controllo persone per IN THREE DAY", response);
          setPeopleInThreeDay(response.data.totalPeoples);
        } catch (error) {
          console.error(
            "Errore durante il recupero delle persone per IN THREE DAY:",
            error
          );
        }
      };
