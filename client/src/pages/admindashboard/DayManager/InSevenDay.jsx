...
const InSevenDay = () => {
  ...
  const [reservationInSevenDay, setReservationInSevenDay] = useState(0);
  const [peopleInSevenDay, setPeopleInSevenDay] = useState(0);
  ...
  useEffect(() => {
    ...
    const totalePersoneInSevenDay = peopleInSevenDay;
    const totaleCostoPerInSevenDay  = (costoPerCliente * totalePersoneInSevenDay ).toFixed(
      0
    );
    setCostPerDay(totaleCostoPerInSevenDay );
  }, [peopleInSevenDay]);
  ...
  useEffect(() => {
    ...
    const totalePersoneInSevenDay  = peopleInSevenDay;
    const totaleProfittoPerInSevenDay  = (
      totalePersoneInSevenDay  * profittoPerCliente
    ).toFixed(0);
    setProfitPerDay(totaleProfittoPerInSevenDay );
  }, [peopleInSevenDay]);

  const fetchInSevenDay = async () => {
    try {
      const response = await axios.get(
        "http://localhost:5000/admindashboard/inSevenDay"
      );
 ...
 const handleEmail = async (prenotazioneEmail) => {
  ...
  //! GET TOTAL RESERVATIONS FROM IN SEVEN DAY
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5000/admindashboard/inSevenDayReservations"
        );
        console.log("controllo prenotazioni per IN SEVEN DAY:", response);

        setReservationInSevenDay(response.data.totalReservationsInSevenDays);
      } catch (error) {
        console.error(
          "Errore durante il recupero delle prenotazioni per IN SEVEN DAY:",
          error
        );
      }
    };
    ...
    //! GET TOTAL PEOPLES FROM IN SEVEN DAY
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5000/admindashboard/inSevenDayPeoples"
        );
        console.log("controllo persone per IN SEVEN DAY:", response);
        setPeopleInSevenDay(response.data.totalPeoples);
      } catch (error) {
        console.error(
          "Errore durante il recupero delle persone per IN SEVEN DAY:",
          error
        );
      }
    };
