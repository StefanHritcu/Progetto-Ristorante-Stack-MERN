...
const InFiveDay = () => {
  ...
  const [reservationInFiveDay, setReservationInFiveDay] = useState(0);
  const [peopleInFiveDay, setPeopleInFiveDay] = useState(0);
  ...
  useEffect(() => {
    ...
    const totalePersoneInFiveDay = peopleInFiveDay;
    const totaleCostoPerInFiveDay = (costoPerCliente * totalePersoneInFiveDay).toFixed(
      0
    );
    setCostPerDay(totaleCostoPerInFiveDay);
  }, [peopleInFiveDay]);
  ...
  useEffect(() => {
    ...
    const totalePersoneInFiveDay = peopleInFiveDay;
    const totaleProfittoPerInFiveDay = (
      totalePersoneInFiveDay * profittoPerCliente
    ).toFixed(0);
    setProfitPerDay(totaleProfittoPerInFiveDay);
  }, [peopleInFiveDay]);

  const fetchInFiveDay = async () => {
    try {
      const response = await axios.get(
        "http://localhost:5000/admindashboard/inFiveDay"
      );
  ...
  //! GET TOTAL RESERVATIONS FROM IN FIVE DAY
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5000/admindashboard/inFiveDayReservations"
        );
        console.log("controllo prenotazioni per IN FIVE DAY:", response);

        setReservationInFiveDay(response.data.totalReservationsInFiveDays);
      } catch (error) {
        console.error(
          "Errore durante il recupero delle prenotazioni per IN FIVE DAY:",
          error
        );
      }
    };
  ...
  //! GET TOTAL PEOPLES FROM IN FIVE DAY
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5000/admindashboard/inFiveDayPeoples"
        );
        console.log("controllo persone per IN FIVE DAY:", response);
        setPeopleInFiveDay(response.data.totalPeoples);
      } catch (error) {
        console.error(
          "Errore durante il recupero delle persone per IN FIVE DAY::",
          error
        );
      }
    };
