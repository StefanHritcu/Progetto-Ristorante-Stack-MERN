...
const Tomorrow = () => {
  ...
  const [reservationTomorrow, setReservationTomorrow] = useState(0);
  const [peopleTomorrow, setPeopleTomorrow] = useState(0);
  ...
  useEffect(() => {
    ...
    const totalePersoneTomorrow = peopleTomorrow;
    const totaleCostoPerTomorrow = (costoPerCliente * totalePersoneTomorrow).toFixed(0)
    setCostPerDay(totaleCostoPerTomorrow);
  }, [peopleTomorrow]);
  ...
  useEffect(() => {
    ...
    const totalePersoneTomorrow = peopleTomorrow;
    const totaleProfittoPerTomorrow = (totalePersoneTomorrow * profittoPerCliente).toFixed(0)
    setProfitPerDay(totaleProfittoPerTomorrow);
  }, [peopleTomorrow]);

  const fetchTomorrowData = async () => {
    try {
      const response = await axios.get(
        "http://localhost:5000/admindashboard/tomorrowDatas"
      );
      ...
      console.error(
        "Errore durante il recupero delle prenotazioni di domani:",
        error
      );
    }
  };
  ...
  //! GET TOTAL RESERVATIONS FROM TOMORROW
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5000/admindashboard/tomorrowReservations"
        );
        console.log("controllo prenotazioni per TOMORROW", response);
        setReservationTomorrow(response.data.length);
      } catch (error) {
        console.error(
          "Errore durante il recupero delle prenotazioni per TOMORROW:",
          error
        );
      }
    };
  ...
  //! GET TOTAL PEOPLES FROM TOMORROW
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5000/admindashboard/tomorrowPeoples"
        );
        console.log("controllo persone per TOMORROW", response);
        setPeopleTomorrow(response.data.totalPeoples);
      } catch (error) {
        console.error(
          "Errore durante il recupero delle persone per TOMORROW:",
          error
        );
      }
    };
