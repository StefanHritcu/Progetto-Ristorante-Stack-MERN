...
const InSixDay = () => {
  ...
  const [reservationInSixDay, setReservationInSixDay] = useState(0);
  const [peopleInSixDay, setPeopleInSixDay] = useState(0);
  ...
  useEffect(() => {
    ...
    const totalePersoneInSixDay = peopleInSixDay;
    const totaleCostoPerInSixDay = (costoPerCliente * totalePersoneInSixDay).toFixed(
      0
    );
    setCostPerDay(totaleCostoPerInSixDay);
  }, [peopleInSixDay]);
  ...
  useEffect(() => {
    ...
    const totalePersoneInSixDay = peopleInSixDay;
    const totaleProfittoPerInSixDay = (
      totalePersoneInSixDay * profittoPerCliente
    ).toFixed(0);
    setProfitPerDay(totaleProfittoPerInSixDay);
  }, [peopleInSixDay]);

  const fetchInSixDay = async () => {
    try {
      const response = await axios.get(
        "http://localhost:5000/admindashboard/inSixDay"
      );
  ...
  //! GET TOTAL RESERVATIONS FROM IN SIX DAY
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5000/admindashboard/inSixDayReservations"
        );
        console.log("controllo prenotazioni per IN SIX DAY:", response);

        setReservationInSixDay(response.data.totalReservationsInSixDays);
      } catch (error) {
        console.error(
          "Errore durante il recupero delle prenotazioni per IN SIX DAY:",
          error
        );
      }
    };
    ...
    //! GET TOTAL PEOPLES FROM IN SIX DAY
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5000/admindashboard/inSixDayPeoples"
        );
        console.log("controllo persone per IN SIX DAY:", response);
        setPeopleInSixDay(response.data.totalPeoples);
      } catch (error) {
        console.error(
          "Errore durante il recupero delle persone per IN SIX DAY:",
          error
        );
      }
    };
